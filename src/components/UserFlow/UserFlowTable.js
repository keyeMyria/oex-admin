...
class UserFlowTable extends React.Component {
  ...
  _renderDataSource(datas) {
    ...
    datas.forEach((data, index) => {
      ...
      dataSource.push({
        key: index,
        id: data.get('id'),
        coinSymbol: data.get('coinSymbol'),
  ...
  callback(key) {
    this.props.changeAction('FlowReducer/userSearchData/type', key);
    ...
    if(this.props.searchData.get('type') === '2') {
      this.props.dispatch(FlowAction.getWithdrawRecordByUser(this.props.searchData.toJS()));
    }
  }
  render() {
    return (
