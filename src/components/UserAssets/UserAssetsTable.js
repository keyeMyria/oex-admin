...
class UserAssetsTable extends React.Component {
  ...
  callback(key) {
    ...
    if(this.props.searchData.get('type') === '1') {
      this.props.dispatch(UserAssetsAction.getRechargeRecordByCoin(this.props.searchData.toJS()));
    }
    if(this.props.searchData.get('type') === '2') {
      this.props.dispatch(UserAssetsAction.getWithdrawRecordByCoin(this.props.searchData.toJS()));
    }
  }
  render() {
    return (
