...
class Bonus extends React.Component {
  ...
  componentWillMount() {
    this.props.dispatch(BonusAction.getBonusInfo({id: this.props.params.id}));
  ...
  _goBackAction = (dispatch: Function) => () => {
    dispatch(push(RoutingURL.BonusList()));
  }
  _goUpdateAction = (dispatch: Function) => (id: string) => {
    dispatch(push(RoutingURL.Bonus(id, true)));
  }
  _updateAction = (dispatch) => (params: {}) => {
    ...
    // this.props.dispatch(BonusAction.updateBonusInfo(param));
  }
  clearBonus() {
    this.props.changeAction('BonusReducer/articleInfo',
    Immutable.fromJS({
      type: '', // 1真题2教材同步
      id: '',
      year: '', // 年份 类型1专有
  ...
  whiteFileAction(param) {
    ...
    result.then(fileInfo => {
      if (fileInfo.fileURL) {
        this.props.changeAction('BonusReducer/articleInfo/ossUrl', fileInfo.fileURL);
...
export default Form.create()(Bonus);
